...
def test_slerp_is_unit_norm(q_pair, t):
    q0, q1 = q_pair
    assert np.isclose(np.linalg.norm(slerp(q0, q1, t)), 1.0), f"t={t}"


def test_slerp_endpoints(q_pair):
    q0, q1 = q_pair
    assert np.isclose(abs(np.dot(slerp(q0, q1, 0.0), q0)), 1.0), "t=0 != q0"
    assert np.isclose(abs(np.dot(slerp(q0, q1, 1.0), q1)), 1.0), "t=1 != q1"


def test_matrix_quaternion_roundtrip(rng):
    """무작위 회전 50개: R -> q -> R 이 원래 행렬로 돌아오는가."""
    from src.rotation import rodrigues as _rod
    for _ in range(50):
        R = _rod(rng.normal(size=3), rng.uniform(0.0, np.pi))
