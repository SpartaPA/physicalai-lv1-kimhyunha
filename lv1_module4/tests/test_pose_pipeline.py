...
def test_camera_to_base_matches_chain(pipeline, rng):
    P = rng.uniform(-0.5, 0.5, size=(100, 3))
    chain = default_chain()
    assert np.allclose(pipeline.camera_to_base(P),
                        chain.transform("base", "camera", P)), "체인과 불일치"
    T = pipeline.T_base_link @ pipeline.T_link_camera
    assert np.allclose(pipeline.camera_to_base(P),
                        transform_points(T, P)), "행렬 곱과 불일치"
...
def test_roundtrip_restores_points(pipeline, rng):
    ...
    assert np.allclose(pipeline.base_to_camera(pipeline.camera_to_base(P)), P), \
        "점군 왕복 실패"
    p0 = P[0]
    assert np.allclose(pipeline.base_to_camera(pipeline.camera_to_base(p0)), p0), \
        "단일 점 왕복 실패"
...
def test_joint_angle_changes_result(pipeline, rng):
    ...
    P = rng.uniform(-0.5, 0.5, size=(50, 3))
    pipeline.set_joint_angle(0.0)
    P0 = pipeline.camera_to_base(P)
    pipeline.set_joint_angle(np.deg2rad(30.0))
    P1 = pipeline.camera_to_base(P)
...
def test_rejects_wrong_shape():
    ...
    with pytest.raises(ValueError):
        PosePipeline(np.eye(3), chain.get("link", "camera"))
