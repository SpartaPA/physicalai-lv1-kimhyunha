...
class PosePipeline:
    ...
    def __init__(self, T_base_link, T_link_camera, joint_axis: str = "z"):
        # TODO: 문제 2-1
        Tbl = np.asarray(T_base_link, dtype=float)
        Tlc = np.asarray(T_link_camera, dtype=float)
        if Tbl.shape != (4, 4):
            raise ValueError(f"T_base_link 는 4x4 여야 합니다. 받은 shape={Tbl.shape}")
        if Tlc.shape != (4, 4):
            raise ValueError(f"T_link_camera 는 4x4 여야 합니다. 받은 shape={Tlc.shape}")
        if joint_axis not in _ROT:
            raise ValueError("joint_axis 는 'x', 'y', 'z' 중 하나여야 합니다.")
        self._T_base_link0 = Tbl
        self._T_link_camera = Tlc
    ...
    def T_base_link(self) -> np.ndarray:
        ...
        """
        # TODO: 문제 2-2 (2-1 동작에 필요하므로 함께 구현. joint_angle=0 이면 기준값 그대로)
        Rj = _ROT[self.joint_axis](self.joint_angle)
        return self._T_base_link0 @ make_T(Rj, [0.0, 0.0, 0.0])

    @property
    def T_link_camera(self) -> np.ndarray:
        """T(link <- camera) — 카메라는 link 에 고정돼 있으므로 관절과 무관하다."""
        # TODO: 문제 2-1
        return np.array(self._T_link_camera, dtype=float, copy=True)
    ...
    def object_pose_in_base(self, T_camera_object) -> np.ndarray:
        ...
        """
        # TODO: 문제 2-1
        Tco = np.asarray(T_camera_object, dtype=float)
        if Tco.shape != (4, 4):
            raise ValueError(f"T_camera_object 는 4x4 여야 합니다. 받은 shape={Tco.shape}")
        return self.T_base_camera @ Tco
