...
def dot(a, b) -> float:
    ...
    b = as_vector(b)
    if a.shape != b.shape:
        raise ValueError(f"차원이 다릅니다. a={a.shape}, b={b.shape}")
    total = 0.0
...
def angle_between(a, b, degrees: bool = True) -> float:
    ...
    nb = norm(b)
    if na == 0.0 or nb == 0.0:
        raise ValueError("영벡터는 사이각을 구할 수 없습니다.")
...
def project(a, b) -> np.ndarray:
    ...
    denom = dot(b, b)
    if denom == 0.0:
        raise ValueError("기준 벡터 b가 영벡터입니다.")
...
def skew(a) -> np.ndarray:
    ...
    a = as_vector(a)
    if a.shape != (3,):
        raise ValueError("3차원 벡터만 입력 가능합니다.")
...
def cross(a, b) -> np.ndarray:
    ...
    if a.shape != (3,) or b.shape != (3,):
        raise ValueError("3차원 벡터만 외적을 계산할 수 있습니다.")
    return skew(a) @ b
...
def plane_normal(P1, P2, P3) -> np.ndarray:
    ...
    n = cross(p2 - p1, p3 - p1)
    if norm(n) == 0.0:
        raise ValueError("세 점이 일직선이라 평면이 하나로 정해지지 않습니다.")
    return n / norm(n)
...
def row_echelon(A, pivoting: bool = True):
    ...
    """
    # TODO: 문제 1-6 / 문제 4
    U = np.array(np.asarray(A, dtype=float), dtype=float, copy=True)
    if U.ndim != 2:
        raise ValueError(f"2차원 행렬이 필요합니다. 받은 shape={U.shape}")
    m, n = U.shape
    tol = max(m, n) * np.finfo(float).eps * max(1.0, float(np.max(np.abs(U)))) if U.size else 0.0
    pivot_cols: list[int] = []
    n_swaps = 0
    r = 0
    for c in range(n):
        if r >= m:
            break
        if pivoting:
            piv = r + int(np.argmax(np.abs(U[r:, c])))
            if abs(U[piv, c]) <= tol:
                continue
            if piv != r:
                U[[r, piv]] = U[[piv, r]]
...
def gauss_eliminate(A, b, pivoting: bool = True, verbose: bool = False):
    ...
    for k in range(n):
        if pivoting:
            piv = k + int(np.argmax(np.abs(Aug[k:, k])))
            if piv != k:
                Aug[[k, piv]] = Aug[[piv, k]]
                if verbose:
                    print(f"[pivoting] row {k} <-> row {piv}")
        if Aug[k, k] == 0.0:
            raise ZeroDivisionError(f"피벗이 0이어서 유일해를 구할 수 없습니다 (col {k}).")
    ...
    for i in range(n - 1, -1, -1):
        if Aug[i, i] == 0.0:
            raise ZeroDivisionError(f"피벗이 0이어서 유일해를 구할 수 없습니다 (col {i}).")
        x[i] = (Aug[i, -1] - dot(Aug[i, i + 1:n], x[i + 1:])) / Aug[i, i]
    return x, steps
...
def inverse_gauss_jordan(A) -> np.ndarray:
    ...
    A = np.asarray(A, dtype=float)
    if A.ndim != 2 or A.shape[0] != A.shape[1]:
        raise ValueError("정사각 행렬만 역행렬을 구할 수 있습니다.")
    n = A.shape[0]
    Aug = np.hstack([A, np.eye(n, dtype=float)])
    for k in range(n):
        piv = k + int(np.argmax(np.abs(Aug[k:, k])))
        if abs(Aug[piv, k]) < 1e-12:
            raise np.linalg.LinAlgError("역행렬이 존재하지 않습니다 (특이 행렬).")
        if piv != k:
            Aug[[k, piv]] = Aug[[piv, k]]
        Aug[k] /= Aug[k, k]
        for i in range(n):
