...
def pca_axes(P):
    ...
    X = P - centroid
    C = (X.T @ X) / (P.shape[0] - 1)
    vals, vecs = np.linalg.eigh(C)  # 대칭행렬 전용
    order = np.argsort(vals)[::-1]  # 내림차순
    eigvals = vals[order]
    axes = vecs[:, order]
    if np.linalg.det(axes) < 0.0:  # 오른손 좌표계 (det=+1)
        axes[:, 2] = -axes[:, 2]
...
def fit_plane_lstsq(P):
    ...
    d = c / np.linalg.norm(np.array([a, b_, -1.0]))
    residuals = P @ n + d
    return n, float(d), residuals
...
def remove_outliers(P, residuals, k: float = 3.0):
    ...
    sigma = 1.4826 * np.median(np.abs(residuals - med))  # MAD (이상치에 강함)
    if sigma == 0.0:
