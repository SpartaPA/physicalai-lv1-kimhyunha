...
def rodrigues(axis, theta: float) -> np.ndarray:
    ...
    """
    # TODO: 문제 2-5
    k = normalize(axis)  # 영벡터면 ValueError, 정규화되지 않은 축도 단위화
    K = skew(k)
    return (np.eye(3) + float(np.sin(theta)) * K
            + (1.0 - float(np.cos(theta))) * (K @ K))
...
def gram_schmidt(A) -> np.ndarray:
    ...
    """
    # TODO: 문제 3-2 (modified Gram-Schmidt: 빼자마자 갱신)
    ...
    for j in range(n):
        ...
        nv = norm(v)
        if nv <= 1e-12:
            raise ValueError(f"{j}번 열이 앞선 열들에 종속이라 직교화할 수 없습니다.")
        Q[:, j] = v / nv
    return Q
...
def axis_angle_from_matrix(R, atol: float = 1e-8):
    ...
    """
    # TODO: 문제 6-4
    # 규약: theta=0 이면 축은 (1,0,0) (회전이 없어 축이 무의미하므로 고정),
    #       theta=pi 이면 sin=0 이라 R-R^T 로 부호를 못 정하므로 eig 벡터 그대로 쓴다.
    R = np.asarray(R, dtype=float)
    cos_theta = max(-1.0, min(1.0, (float(np.trace(R)) - 1.0) / 2.0))
    angle = float(math.acos(cos_theta))
    sin_theta = math.sin(angle)
    if abs(sin_theta) > 1e-6:
        # R - R^T = 2 sin(theta) [k]_x 이므로 부호까지 맞게 복원된다
        axis = np.array([R[2, 1] - R[1, 2],
                         R[0, 2] - R[2, 0],
                         R[1, 0] - R[0, 1]], dtype=float) / (2.0 * sin_theta)
        return normalize(axis), angle
    if angle < 1e-6:
        return np.array([1.0, 0.0, 0.0], dtype=float), 0.0
    eigvals, eigvecs = np.linalg.eig(R)  # 축 복원 목적이므로 허용
...
def quaternion_from_axis_angle(axis, angle: float) -> np.ndarray:
    ...
    """
    # TODO: 문제 6-5
    k = normalize(axis)  # 영벡터면 ValueError
    half = float(angle) / 2.0
    s = math.sin(half)
    q = np.array([k[0] * s, k[1] * s, k[2] * s, math.cos(half)], dtype=float)
    return q / norm(q)
