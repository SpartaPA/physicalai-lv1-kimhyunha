...
def linear_interp(t_wp, q_wp, t) -> np.ndarray:
    ...
    if q_wp.ndim == 1:
        return np.array([np.interp(ti, t_wp, q_wp) for ti in t])
    return np.stack([np.interp(t, t_wp, q_wp[:, d]) for d in range(q_wp.shape[1])],
                    axis=1)
...
def quintic_profile(t, t0: float, tf: float, q0, qf,
    ...
    if scalar:
        d = float(qfa - q0a)
        return q0a + d * s, d * sd, d * sdd
    d = (qfa - q0a)[None, :]
    return q0a + d * s[:, None], d * sd[:, None], d * sdd[:, None]
...
def finite_diff(y, t) -> np.ndarray:
    ...
    """
    # TODO: 문제 4-2
    return np.gradient(np.asarray(y, dtype=float), np.asarray(t, dtype=float),
                       axis=0)
