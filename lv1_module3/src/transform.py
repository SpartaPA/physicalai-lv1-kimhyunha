...
def inv_T_batch(Ts) -> np.ndarray:
    ...
    """
    # TODO: 문제 5-4 (반복문 없이 배치 연산)
    Ts = np.asarray(Ts, dtype=float)
    Rt = np.swapaxes(Ts[:, :3, :3], 1, 2)
    t = Ts[:, :3, 3]
    Ti = np.zeros_like(Ts)
    Ti[:, :3, :3] = Rt
    Ti[:, :3, 3] = -np.einsum("nij,nj->ni", Rt, t)
    Ti[:, 3, 3] = 1.0
    return Ti
...
def to_homogeneous(P, w: float = 1.0) -> np.ndarray:
    ...
    if P.shape == (3,):
        return np.array([P[0], P[1], P[2], float(w)], dtype=float)
    P = np.atleast_2d(P)
    if P.shape[1] != 3:
        raise ValueError(f"(3,) 또는 (N,3) 좌표가 필요합니다. 받은 shape={P.shape}")
    return np.hstack([P, np.full((P.shape[0], 1), float(w))])
...
def transform_points(T, P, w: float = 1.0) -> np.ndarray:
    ...
    """
    # TODO: 문제 5-2 / 6-2 (반복문 없이: P_h @ T.T)
    T = np.asarray(T, dtype=float)
    P = np.asarray(P, dtype=float)
    single = P.shape == (3,)
    P_h = np.atleast_2d(to_homogeneous(P, w=w))
    out = (P_h @ T.T)[:, :3]
